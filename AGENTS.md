...
- 默认模型回退为 `gpt-image-2`，同时允许用户从完整探测模型列表里手动选择生图模型和 AI 改写模型。
- 不要把“未按启发式识别出文本模型/图片模型”当成不可用；很多中转站会把生图能力集成在普通模型名或自定义别名下。
...
- AI 改写是可选增强，优先使用用户选择的 AI 改写模型；未选择时再使用启发式文本模型或完整模型列表兜底。
